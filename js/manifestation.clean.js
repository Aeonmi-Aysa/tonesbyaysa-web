(function (global) {
  class ManifestationToolkit {
    constructor(options = {}) {
      ...
      this.composerBaths = this.loadComposerBaths();
    ...
    cacheElements() {
      ...
      if (!this.elements.timer) {
        console.warn('Timer element not found - session timer will not display');
      }
    ...
    loadComposerBaths() {
      try {
        const raw = localStorage.getItem('manifestationComposerBaths');
        if (raw) return JSON.parse(raw);
      } catch (err) {
        console.warn('Unable to load composer baths', err);
      }
      return [];
    }

    persistComposerBaths() {
      try {
        localStorage.setItem('manifestationComposerBaths', JSON.stringify(this.composerBaths || []));
      } catch (err) {
        console.warn('Unable to persist composer baths', err);
      }
    }

    deleteComposerBath(bathId) {
      if (!bathId) return;
      this.composerBaths = (this.composerBaths || []).filter(bath => bath.id !== bathId);
      this.persistComposerBaths();
      this.renderBathGrid();
      this.showToast('Saved bath removed.');
    }

    recordSession(minutes, intentionId = null) {
      try {
        ...
        this.state.sessionHistory.push({
          ...
          duration: minutes,
          intentionId: intentionId || this.state.currentIntention?.title || 'general',
          category: this.state.currentIntention?.category || 'general'
        });
    ...
    renderRecommendations() {
      ...
      items.forEach(preset => {
        ...
        card.innerHTML = `
          ...
          <div class="recommendation-action">${preset.type === 'single' || preset.type === 'binaural' ? 'Play' : 'Launch Bath'}</div>
        `;
    ...
    renderBathGrid() {
      ...
      baths.forEach(bath => {
        ...
        const deleteBtn = bath.source === 'composer' 
          ? `<button class="btn-manifest btn-delete-bath" type="button" data-bath-id="${bath.id}" style="margin-left:0.5rem;padding:0.35rem 0.65rem;font-size:0.85rem;background:#8b2c2c;border-color:#c44545;">Delete</button>`
          : '';
        card.innerHTML = `
      ...
      this.elements.baths.querySelectorAll('.btn-delete-bath').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          const bathId = btn.dataset.bathId;
          if (confirm('Remove this saved bath from your collection?')) {
            this.deleteComposerBath(bathId);
          }
        });
      });
    }
    ...
    importComposerBath(bath) {
      ...
      // Remove limit (.slice(0, 8)) to allow unlimited saved baths + persist to storage
      this.composerBaths = [normalized, ...this.composerBaths.filter(item => item.id !== normalized.id)];
      this.persistComposerBaths();
      this.renderBathGrid();
      this.showToast('Composer bath saved to Manifestation tab.');
    ...
    startSession() {
      try {
        ...
        if (!this.elements.timer) {
          console.error('Timer element not found - cannot start session');
          this.showToast('Session setup error - timer not found');
          return;
        }
        ...
        // Trigger immediate update to show 00:00
        this.updateTimerDisplay(resumeOffset);

        // Set up interval for counting - use arrow function to preserve 'this'
        const self = this;
        this.session.timer = setInterval(function() {
          try {
            if (self.session.isActive && self.session.start) {
              const elapsed = Date.now() - self.session.start;
        ...
        this.updateSessionButtons();
        console.log('Session started - timer ID:', this.session.timer);
        this.showToast('Session started - timer running');
      } catch (err) {
        console.error('Error starting session:', err);
        this.session.isActive = false;
        this.session.timer = null;
        this.updateSessionButtons();
        this.showToast('Session start failed');
      }
    }
    ...
    pauseSession() {
      try {
        ...
        this.updateTimerDisplay(this.session.pausedOffset);
        
        this.updateSessionButtons();
        console.log('Session paused at:', this.session.pausedOffset, 'ms');
        this.showToast('Session paused');
      } catch (err) {
        console.error('Error pausing session:', err);
        this.showToast('Pause failed');
      }
    }
    ...
    stopSession(markComplete = false) {
      try {
        ...
        if (markComplete) {
          ...
          this.renderStats();
          this.renderAchievements();
          console.log('Session saved:', minutes, 'minutes');
    ...
    tickSession() {
      try {
        if (!this.session.isActive || !this.session.start) {
          return;
        }
        const elapsed = Date.now() - this.session.start;
        this.updateTimerDisplay(elapsed);
      } catch (err) {
        console.error('Error in tickSession:', err);
      }
    }
    ...
    updateTimerDisplay(elapsedMs) {
      try {
        ...
        // Always update to ensure smooth counting
        this.elements.timer.textContent = newTime;
      } catch (err) {
    ...
    updateSessionButtons() {
      try {
        if (!this.elements.startBtn || !this.elements.pauseBtn || !this.elements.stopBtn) {
          console.warn('Session buttons not found in DOM');
          return;
        }
        ...
        console.error('Error updating session buttons:', err);
      }
    }
    ...
    async syncFromSupabase() {
      ...
      try {
        ...
        this.state.currentIntention = data.current_intention || this.state.currentIntention;
        if (data.stats) {
